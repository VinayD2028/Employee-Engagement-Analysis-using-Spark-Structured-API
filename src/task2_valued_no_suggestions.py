...
def identify_valued_no_suggestions(df):
    ...
    count_valued = valued_no_suggestions_df.count()
    total_employees = df.count()
    proportion = (count_valued / total_employees) * 100 if total_employees > 0 else 0
    # 4. Return the results.
    return count_valued, round(proportion, 2)
